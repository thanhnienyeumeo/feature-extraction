...
def normalize_timestamp(s):
    """
    Normalize timestamp string to handle encoding issues (e.g., latin-1 artifacts).
    ...
    if not isinstance(s, str):
        # If it's already a datetime, return formatted string
    ...
    # Remove non-ASCII and non-printable characters that might come from encoding issues
    s = re.sub(r'[^\d/: ]', '', s)
    s = re.sub(r'\s+', ' ', s)
    ...
    # Validate that m, day, y are numeric
    try:
        int(m)
        int(day)
        int(y)
    ...
    # Validate time part has correct format
    t = time_part.split(':')
    if len(t) != 3:
        return None
    try:
    ...
    return f"{m.zfill(2)}/{day.zfill(2)}/{y} {time_part}"


def safe_strptime(s, fmt='%m/%d/%Y %H:%M:%S'):
    """
    Safely parse a timestamp string, returning None on failure.
    """
...
def sanitize_string(s):
    ...
    if not isinstance(s, str):
        return s
    # Encode to latin-1 then decode to utf-8 with error handling
    try:
...
def time_convert(inp, mode, real_sd = '2010-01-02', sd_monday= "2009-12-28"):
    ...
    """
    # For modes that expect timestamp strings, normalize first
    if mode in ['t2e', 't2dt', 't2date', 't2wn']:
        inp = normalize_timestamp(inp)
        if inp is None:
            return None
...
def add_action_thisweek(act, columns, lines, act_handles, week_index, stop, firstdate, dname = 'r5.2'):
    ...
    while True:
        ...
        # Handle lines with extra commas in content field - join extra parts back
        if len(tmp) > n_cols:
            tmp = tmp[:n_cols-1] + [','.join(tmp[n_cols-1:])]
        # Skip lines with too few columns
        elif len(tmp) < n_cols:
            lines[act] = act_handles[act].readline()
            continue
        
        # Normalize the timestamp before processing
        normalized_ts = normalize_timestamp(tmp[1])
        if normalized_ts is None:
            # Skip rows with invalid timestamps
            lines[act] = act_handles[act].readline()
            continue
        
        week = time_convert(normalized_ts, 't2wn', real_sd=firstdate)

        if week is None:
            # Skip rows with invalid week calculation
            lines[act] = act_handles[act].readline()
            continue

        if week == week_index:
            # Store the normalized timestamp
            tmp[1] = normalized_ts
...
def combine_by_timerange_pandas(dname = 'r4.2'):
    ...
    # Get the first valid timestamp from http.csv to determine the start date
    firstdate = None
    with open('http.csv', 'r', encoding='latin-1') as f:
        next(f)  # Skip header
        for line in f:
            parts = line.split(',')
            if len(parts) > 1:
                firstdate = time_convert(parts[1], 't2dt')
                if firstdate is not None:
                    break
    
    if firstdate is None:
        raise ValueError("Could not find a valid timestamp in http.csv to determine start date")
    ...
    stop = {}
    for act in allacts:
        act_handles[act] = open(act+'.csv','r', encoding='latin-1')
    ...
    while sum(stop.values()) < 5:
        ...
        for act in allacts:
            ...
            if len(df) > 0:
                thisweek_dfs.append(df)
        
        if thisweek_dfs:
            thisweekdf = pd.concat(thisweek_dfs, sort=False)
        else:
            thisweekdf = pd.DataFrame()
        
        # Safe datetime parsing - timestamps are already normalized in add_action_thisweek
        if len(thisweekdf) > 0:
            thisweekdf['date'] = thisweekdf['date'].apply(safe_strptime)
            # Drop rows where date parsing failed (should be rare since we normalized earlier)
            thisweekdf = thisweekdf.dropna(subset=['date'])
            
            # Sanitize all string columns to ensure valid UTF-8 encoding for pickle
            for col in thisweekdf.columns:
                if thisweekdf[col].dtype == 'object':
                    thisweekdf[col] = thisweekdf[col].apply(sanitize_string)
...
def get_mal_userdata(data = 'r4.2', usersdf = None):
    ...
    # Use safe_strptime for robust datetime parsing
    listmaluser['start'] = listmaluser['start'].apply(safe_strptime)
    listmaluser['end'] = listmaluser['end'].apply(safe_strptime)
...
def email_process(act, data = 'r4.2', separate_send_receive = True):
    to_field = act['to'] if isinstance(act['to'], str) else ''
    receivers = to_field.split(';') if to_field else []
    if isinstance(act['cc'], str) and act['cc']:
        receivers = receivers + act['cc'].split(";")
    if isinstance(act['bcc'], str) and act['bcc']:
    ...
    exbccmail = 0
    content = act['content'] if isinstance(act['content'], str) else ''
    email_text_len = len(content)
    email_text_nwords = content.count(' ') + 1
    ...
    if data in ['r5.1','r5.2','r6.1','r6.2']:
        ...
        att_field = act['att'] if isinstance(act['att'], str) else ''
        atts = att_field.split(';') if att_field else []
        n_atts = len(atts)
        size_atts = 0
        ...
        return [send_mail, receive_mail, n_des, n_atts, Xemail, n_exdes, 
                n_bccdes, exbccmail, safe_int(act['size']), email_text_len, 
                email_text_nwords] + att_types + att_sizes
    elif data in ['r4.1','r4.2']:
        return [n_des, safe_int(act['#att']), Xemail, n_exdes, n_bccdes, exbccmail, 
                safe_int(act['size']), email_text_len, email_text_nwords]
        
def http_process(act, data = 'r4.2'): 
    # basic features:
    url = act['url/fname'] if isinstance(act['url/fname'], str) else ''
    url_len = len(url)
    url_depth = max(0, url.count('/') - 2)
    content = act['content'] if isinstance(act['content'], str) else ''
    content_len = len(content)
    content_nwords = content.count(' ') + 1
    
    # Extract domain name - handle URLs with or without trailing slash
    domain_match = re.findall(r"//([^/]+)", url)
    if domain_match:
        domainname = domain_match[0].replace("www.", "")
    else:
        domainname = ""
...
def file_process(act, complete_ul = None, data = 'r4.2', filetype = 'act'):
    if filetype == 'act':
        fname = act['url/fname'] if isinstance(act['url/fname'], str) else ''
        fname_parts = fname.split(".")
        ftype = fname_parts[1] if len(fname_parts) > 1 else 'unknown'
        first_char = fname[0] if fname else ''
        disk = 1 if first_char == 'C' else 0
        if first_char == 'R': disk = 2
        file_depth = fname.count('\\')
    elif filetype == 'att': #attachments
        act_str = act if isinstance(act, str) else ''
        parts = act_str.split('.')
        tmp = parts[1] if len(parts) > 1 else ''
        ftype = tmp[:tmp.find('(')] if '(' in tmp else 'unknown'
        try:
            attsize = int(tmp[tmp.find("(")+1:tmp.find(")")]) if '(' in tmp and ')' in tmp else 0
        except ValueError:
            attsize = 0
...
if __name__ == "__main__":
    ...
    #make temporary folders (use makedirs with exist_ok to avoid errors)
    for x in ["tmp", "ExtractedData", "DataByWeek", "NumDataByWeek"]:
    ...
    for mode in ['week','day','session']:
        ...
        # Session mode is memory-intensive - run sequentially to avoid OOM
        if mode == 'session':
            print("  Processing session data sequentially to avoid memory issues...")
            for idx, week in enumerate(weekRange):
                to_csv(week, mode, dname, ul, uf_dict, list_uf, subsession_mode)
                if (idx + 1) % 10 == 0:
                    gc.collect()
                    print(f"  Session progress: {idx + 1}/{len(weekRange)} weeks done")
            print(f"  Session progress: {len(weekRange)}/{len(weekRange)} weeks done")
        else:
        ...
        gc.collect()  # Clean up memory after each mode
