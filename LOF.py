...
# coding: utf-8
"""
Anomaly Detection using Local Outlier Factor (LOF) - Supports CSV and PKL input
...
Usage:
    python LOF.py /path/to/weekr4.2.csv
    python LOF.py /path/to/week-r5.2-percentile30.pkl
"""
...
# ============================================================================
if len(sys.argv) < 2:
    print("\nUsage: python LOF.py <path_to_data>")
    print("Example: python LOF.py /home/user/r4.2/ExtractedData/weekr4.2.csv")
    sys.exit(1)
...
# ============================================================================
# STEP 7: Train Local Outlier Factor (LOF)
# ============================================================================
print('\n*****Training Local Outlier Factor (LOF)...')
# LOF model with novelty=True for detecting anomalies in new data
# n_neighbors: number of neighbors to use for k-neighbors queries
# contamination: proportion of outliers in the data set
# novelty=True: allows using predict on new unseen data
...
anomaly_scores_not_insiders = -lof.score_samples(xTest)
...
lof_with_insider = LocalOutlierFactor(
    n_neighbors=20,
    contamination='auto',
    novelty=True,
    n_jobs=-1
)
lof_with_insider.fit(xTrain_scaled)
print('Training completed for insider in training')
anomaly_scores_insiders = -lof_with_insider.score_samples(xTest_scaled_with_insider)
...
# ENSEMBLE LOF and MLP
anomaly_scores_insiders = (
    anomaly_scores_insiders_mlp + anomaly_scores_insiders
) / 2 

print('\n' + '=' * 80)
print('RESULTS FOR INSIDER IN TRAINING (ENSEMBLE: LOF + MLP)')
print('=' * 80)
evaluate_results(anomaly_scores_insiders, yTestBin)

anomaly_scores_not_insiders = (
    anomaly_scores_not_insiders_mlp + anomaly_scores_not_insiders
) / 2

print('\n' + '=' * 80)
print('RESULTS FOR INSIDER NOT IN TRAINING (ENSEMBLE: LOF + MLP)')
print('=' * 80)
evaluate_results(anomaly_scores_not_insiders, yTestBin)